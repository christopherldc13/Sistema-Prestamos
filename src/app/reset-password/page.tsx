...
import { Lock, ShieldCheck, CheckCircle2, AlertCircle, Eye, EyeOff, Sun, Moon } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useTheme } from "@/components/ThemeProvider";

// useSearchParams debe leerse dentro de un límite Suspense en el App Router
function TokenReader({ onToken }: { onToken: (t: string | null) => void }) {
    const searchParams = useSearchParams();
    useEffect(() => {
        onToken(searchParams.get("token"));
...
export default function ResetPasswordPage() {
    ...
    return (
        <div className="login-viewer">
            ...
            >
                <header className="login-header">
                    ...
                    <p className="login-subtitle">Elige una contraseña nueva y segura</p>
                </header>
                ...
                {token === undefined ? null : token === null ? (
                    <div className="fp-sent-box">
                        ...
                        <Link href="/forgot-password" className="login-btn-pro fp-back-btn">Solicitar nuevo enlace</Link>
                    </div>
                ) : done ? (
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="fp-sent-box">
                        <CheckCircle2 size={32} color="#10b981" />
                        <p>Tu contraseña se actualizó correctamente. Te llevamos al inicio de sesión...</p>
                    </motion.div>
                ) : (
                    <>
                        <AnimatePresence>
                        ...
                        <form onSubmit={handleSubmit} className="login-form">
                            ...
                            <motion.button
                                ...
                                whileTap={{ scale: 0.98 }}
                            >
                                {loading ? <span className="loader-icon" /> : <span>Guardar nueva contraseña</span>}
                            </motion.button>
                        </form>
                    </>
                )}
        ...
        .login-input-wrap input {
          ...
          border-radius: 10px; padding: 0.875rem 3rem 0.875rem 3rem; color: var(--text-main);
        ...
        .toggle-password {
          ...
          color: var(--text-faint); cursor: pointer; padding: 4px; display: flex;
          align-items: center; border-radius: 6px; transition: color 0.2s;
        }
